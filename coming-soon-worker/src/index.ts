...
interface Env {
  /** Service binding to the real "kostka-tcg" worker (see wrangler.jsonc). */
  ORIGIN: Fetcher;
  /** Set via `wrangler secret put PREVIEW_SECRET --config wrangler.jsonc`. */
  PREVIEW_SECRET: string;
}

const COOKIE_NAME = "kostka_preview";
const COOKIE_MAX_AGE = 60 * 60 * 24 * 365; // 1 year

function hasValidCookie(request: Request, secret: string): boolean {
  const cookieHeader = request.headers.get("Cookie") ?? "";
  return cookieHeader
    .split(";")
    .map((part) => part.trim())
    .includes(`${COOKIE_NAME}=${secret}`);
}
...
export default {
  async fetch(request: Request, env: Env): Promise<Response> {
    ...
    // Fail closed: without a configured secret nobody can unlock the site,
    // which is the safer default for a work-in-progress domain.
    if (!secret) {
...
const KOSTKA_ICON_SVG = `<svg xmlns="http://www.w3.org/2000/svg" viewBox="0 0 121.78 120.08" width="72" height="72" role="img" aria-label="KOSTKA TCG">
...
<title>KOSTKA TCG – připravujeme nový web</title>
<style>
